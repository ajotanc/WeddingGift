...
- Composable (`useTenant.ts`): Carrega os dados do casal com base no `:slug` da URL e injeta a `primaryColor` dinamicamente no root do CSS do Tailwind.
...
- **Configuração do Firebase (.env):** O arquivo `.env` deve residir OBRIGATORIAMENTE na raiz do projeto (nunca na pasta `src/`) e as chaves não podem ter vírgulas no final. O arquivo `src/firebase.ts` deve conter _fallback dummy strings_ para evitar que a SDK exploda a importação antes do arquivo `.env` ser lido no build/deploy.
