...
- **Tipagem Estrita (Zero "any"):** Toda adição de campos novos em formulários (ex: `imageUrl`) deve refletir em atualização imediata nas interfaces do `src/types/index.ts` (ex: `Product` e `SerperItem`), garantindo IntelliSense e evitando asserções como `(product as any)`.
