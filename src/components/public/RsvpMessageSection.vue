...
<template>
	...
	<section class="grid grid-cols-1 lg:grid-cols-12 gap-12 lg:gap-16 items-start">
		...
		<div id="rsvp" class="lg:col-span-7 space-y-8 scroll-mt-16">
			<!-- Column Header -->
			<div class="border-b border-[#E8E2DD]/60 pb-6 text-left">
				<span class="text-[10px] font-bold tracking-[0.25em] uppercase" :style="{ color: tenant?.primary_color || '#ec4899' }">Confirmação</span>
				<h2 class="text-3xl font-serif text-[#1E1A17] mt-1 font-semibold">Presença no Evento</h2>
				<p class="text-[#5A4F4A] font-light leading-relaxed mt-2 text-sm">
					Ficaremos imensamente felizes em celebrar esse momento único com você. Por favor, confirme suas informações abaixo.
				</p>
			</div>

			<!-- Form Container -->
			<div class="bg-white border border-[#E8E2DD] p-8 rounded-[2rem] shadow-[0_12px_40px_rgba(0,0,0,0.015)]">
				<!-- Already Submitted State -->
				<div v-if="existingRsvp && !isEditingRsvp" class="text-center py-4">
					<div class="w-14 h-14 bg-emerald-50 text-emerald-600 rounded-full flex items-center justify-center mx-auto mb-6 border border-emerald-100">
						<svg class="w-6 h-6" fill="none" stroke="currentColor" viewBox="0 0 24 24">
							<path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M5 13l4 4L19 7" />
						</svg>
					</div>
					<h3 class="text-2xl font-serif text-[#1E1A17] mb-2 font-semibold">
						{{ existingRsvp.status === 'confirmed' ? 'Presença Confirmada!' : 'Não Poderá Ir' }}
					</h3>
					<p class="text-[#5A4F4A] font-light text-sm mb-6 max-w-md mx-auto">
					...
					<div
						...
						class="text-xs text-[#5A4F4A] mb-6 space-y-2 bg-slate-50 p-4 rounded-2xl border border-[#E8E2DD] text-left">
						<p v-if="existingRsvp.companions_names && existingRsvp.companions_names.length > 0" class="flex gap-2">
							<strong class="text-[#1E1A17]">Acompanhantes:</strong> 
							<span>{{ existingRsvp.companions_names.join(', ') }}</span>
						</p>
						<p v-if="existingRsvp.dietary_restrictions" class="flex gap-2">
							<strong class="text-[#1E1A17]">Restrições Alimentares:</strong>
							<span>{{ existingRsvp.dietary_restrictions }}</span>
						</p>
					</div>
					<div v-if="existingRsvp.message" class="relative max-w-md mx-auto my-6 p-5 bg-slate-50/80 rounded-2xl border border-[#E8E2DD] italic font-serif text-[#5A4F4A] text-sm">
					...
					<Button variant="outline" class="rounded-full px-8" :style="{ borderColor: tenant?.primary_color || '#ec4899', color: tenant?.primary_color || '#ec4899' }" @click="isEditingRsvp = true">
						Alterar Resposta
					</Button>
				</div>
				...
				<div v-else-if="!tenant?.is_premium && rsvps.length >= 20 && !existingRsvp" class="text-center py-6">
					<div class="w-12 h-12 bg-amber-50 rounded-full flex items-center justify-center mx-auto mb-4 border border-[#E8E2DD]" :style="{ color: tenant?.primary_color || '#ec4899' }">
					...
					<p class="text-xs font-semibold" :style="{ color: tenant?.primary_color || '#ec4899' }">
				...
				<form v-else @submit.prevent="submitRsvp" class="space-y-6 text-left">
					<div class="space-y-5">
						...
						<div v-if="status === 'confirmed'" class="grid grid-cols-2 gap-4 animate-in fade-in duration-300">
							<FormGroup label="Adultos" :error="errors.totalAdults">
								<Input v-model.number="totalAdults" type="number" min="1"
									class="rounded-xl border-[#E8E2DD] shadow-sm focus-visible:ring-0 focus-visible:border-primary bg-slate-50/50 h-11 text-sm text-[#1E1A17] font-medium" />
							</FormGroup>
						...
						<FormGroup label="Você irá ao evento?">
							<Select v-model="status">
								<SelectTrigger
									class="w-full h-11 px-4 bg-slate-50/50 border border-[#E8E2DD] rounded-xl text-sm font-light text-[#5A4F4A] focus:ring-0 focus:border-primary text-left">
								...
								<div class="space-y-2">
									<div v-for="(_companion, idx) in companionsList" :key="idx" class="flex items-center gap-2">
										<Input v-model="companionsList[idx]" :placeholder="`Nome do acompanhante ${idx + 1}`"
											class="rounded-xl border-[#E8E2DD] shadow-sm focus-visible:ring-0 focus-visible:border-primary bg-slate-50/50 h-11 text-sm text-[#1E1A17]" />
									</div>
								</div>
							</FormGroup>
						</div>
						...
						<div class="flex items-start gap-3 py-1">
							...
							<label for="accept-rsvp-terms"
								...
								Autorizo o tratamento de meus dados em conformidade com os <a href="/terms" target="_blank"
									class="underline font-semibold" :style="{ color: tenant?.primary_color || '#ec4899' }">Termos de Uso</a> e <a href="/privacy" target="_blank"
									class="underline font-semibold" :style="{ color: tenant?.primary_color || '#ec4899' }">Política de Privacidade</a> (LGPD).
							</label>
						</div>
					...
					<div class="flex flex-col gap-2 pt-2">
						<Button type="submit" 
							...
							class="w-full h-11 text-white hover:brightness-105 active:scale-[0.98] transition-all rounded-xl font-semibold uppercase tracking-wider text-xs"
		...
		<div id="messages" class="lg:col-span-5 space-y-8 scroll-mt-16">
			<!-- Column Header -->
			<div class="border-b border-[#E8E2DD]/60 pb-6 text-left">
				<span class="text-[10px] font-bold tracking-[0.25em] uppercase" :style="{ color: tenant?.primary_color || '#ec4899' }">Afeto</span>
				<h2 class="text-3xl font-serif text-[#1E1A17] mt-1 font-semibold">Mural de Recados</h2>
				<p class="text-[#5A4F4A] font-light leading-relaxed mt-2 text-sm">
			...
			<div class="bg-white border border-[#E8E2DD] p-6 rounded-[2rem] shadow-[0_12px_40px_rgba(0,0,0,0.015)] flex flex-col gap-4">
				
				<!-- Sender header info -->
				<div class="flex items-center gap-3">
					<img v-if="authStore.guest?.photo_url" :src="authStore.guest.photo_url" alt="Foto"
						referrerpolicy="no-referrer" class="w-10 h-10 rounded-full border border-[#E8E2DD] shadow-sm" />
					<div v-else class="w-10 h-10 rounded-full flex items-center justify-center font-serif text-sm font-semibold select-none"
						:style="{ backgroundColor: (tenant?.primary_color || '#ec4899') + '1a', border: '1px solid ' + (tenant?.primary_color || '#ec4899') + '33', color: tenant?.primary_color || '#ec4899' }">
					...
					<div class="flex flex-col text-left">
						...
						<strong class="text-sm font-serif font-bold text-[#1E1A17] mt-0.5">{{ authStore.guest?.name }}</strong>
					</div>
				</div>

				<!-- Content input -->
				<Textarea v-model="messageContent"
					class="w-full h-28 rounded-2xl bg-slate-50/50 border border-[#E8E2DD] focus-visible:ring-0 focus-visible:border-primary placeholder:text-slate-400 text-sm font-light p-4 resize-none leading-relaxed text-[#1E1A17]"
					placeholder="Escreva uma mensagem de carinho para os noivos..." />

				<!-- Submit button (fixed location to prevent layout shift) -->
				<Button @click="submitMessage" 
					:style="messageContent.trim() ? { backgroundColor: tenant?.primary_color || '#1E1A17', color: '#fff' } : { backgroundColor: '#E8E2DD', color: '#FAF8F6', cursor: 'not-allowed' }"
					class="w-full h-11 rounded-xl text-xs font-semibold uppercase tracking-wider transition-all duration-300">
					Publicar Recado
				</Button>
			</div>

			<!-- Messages Carousel -->
			<div v-if="messages.length > 0" class="space-y-6 pt-4 text-left">
				<h3 class="text-xs uppercase tracking-widest font-bold" :style="{ color: tenant?.primary_color || '#ec4899' }">Recados Enviados</h3>

				<Carousel class="w-full relative cursor-grab active:cursor-grabbing pb-6"
					...
					<CarouselContent class="py-2">
						<CarouselItem v-for="(msg, index) in messages" :key="msg.$id"
							class="basis-full select-none">
							<div
								class="w-full p-6 rounded-[1.5rem] relative overflow-hidden group transition-all duration-300 flex flex-col gap-4 border"
								:class="index % 2 === 0 ? 'bg-white border-[#E8E2DD]' : 'border-transparent text-white'"
								:style="index % 2 !== 0 ? { backgroundColor: tenant?.primary_color || '#ec4899' } : {}">

								<!-- Quote Icon & Likes Header -->
								<div class="flex items-center justify-between z-10">
									<svg class="w-6 h-6 fill-current shrink-0" viewBox="0 0 24 24"
										:style="{ color: index % 2 === 0 ? (tenant?.primary_color || '#ec4899') + '4d' : 'rgba(255, 255, 255, 0.4)' }">
									...
									<div class="flex items-center gap-1">
										<Button variant="ghost" @click="toggleLike(msg)"
											...
											:style="{ color: index % 2 === 0 ? tenant?.primary_color || '#ec4899' : '#fff' }">
								...
								<p class="font-serif italic leading-relaxed text-sm whitespace-pre-wrap break-words z-10"
									:class="index % 2 === 0 ? 'text-[#5A4F4A]' : 'text-white/90'">
									"{{ msg.content }}"
								</p>

								<!-- Sender Info footer -->
								<div class="flex items-center gap-3 pt-3 border-t mt-1 z-10"
									:class="index % 2 === 0 ? 'border-[#E8E2DD]/50' : 'border-white/20'">
									<img v-if="msg.guest.photo_url" :src="msg.guest.photo_url" referrerpolicy="no-referrer"
										class="w-8 h-8 shrink-0 rounded-full border shadow-xs"
										:class="index % 2 === 0 ? 'border-[#E8E2DD]' : 'border-white/20'" />
									<div v-else class="w-8 h-8 shrink-0 rounded-full flex items-center justify-center text-xs font-serif font-bold"
										:style="index % 2 === 0 ? { backgroundColor: (tenant?.primary_color || '#ec4899') + '26', border: '1px solid ' + (tenant?.primary_color || '#ec4899') + '33', color: tenant?.primary_color || '#ec4899' } : { backgroundColor: 'rgba(255, 255, 255, 0.15)', border: '1px solid rgba(255, 255, 255, 0.25)', color: '#fff' }">
										{{ msg.guest.name?.charAt(0).toUpperCase() }}
									</div>
									<div class="flex flex-col text-left min-w-0">
										<p class="text-xs font-serif font-bold truncate leading-none"
											:class="index % 2 === 0 ? 'text-[#1E1A17]' : 'text-white'">{{ msg.guest.name }}</p>
										<p class="text-[9px] font-sans font-light mt-1 truncate"
											:class="index % 2 === 0 ? 'text-slate-400' : 'text-white/70'">{{ dayjs(msg.$createdAt).format('DD/MM/YYYY') }}</p>
									</div>
