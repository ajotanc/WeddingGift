...
<template>
  <div class="relative">
    ...
    <div v-else class="relative rounded-2xl border border-slate-200 p-3 bg-white shadow-sm flex items-center gap-4">
      ...
      <button type="button" v-if="!uploading" @click="removeFile" class="p-2 text-slate-400 hover:text-red-500 bg-transparent border-0 outline-none transition-colors cursor-pointer">
        <X class="w-5 h-5" />
      </button>
    </div>

    <teleport to="body">
      <div v-if="isZoomed" @click.stop="closeZoomOverlay" @pointerdown.stop @pointerup.stop @mousedown.stop @mouseup.stop
