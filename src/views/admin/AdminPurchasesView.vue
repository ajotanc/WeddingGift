...
const { purchases, tenant } = useTenant();
...
<template>
  <div class="space-y-6">
    ...
    <div v-else class="space-y-4">
      <!-- Surprise mode notification banner -->
      <div v-if="!isEventPassedOrToday"
        class="flex items-center gap-3 p-4 bg-amber-50/80 border border-amber-200/60 rounded-2xl text-amber-800 text-xs">
        <Sparkles class="w-4 h-4 text-amber-600 shrink-0" />
        <p>
          <strong>Modo Surpresa Ativo:</strong> Os nomes dos convidados que presentearam ficam ocultos até o momento do
          evento para manter a surpresa (revelação em {{ revealDateFormatted }}).
        </p>
      </div>
                ...
                <td class="p-4">
                  <div v-if="isEventPassedOrToday">
                    <p class="text-sm font-medium text-slate-900">
                      {{ purchase.guest?.name || 'Convidado' }}
                    </p>
                  </div>
                  <p v-else class="text-sm font-medium text-slate-900 group relative inline-block cursor-help">
