    ...
    <header class="bg-white border-b border-zinc-200 sticky top-0 z-40 w-full">
      ...
      <nav class="md:hidden flex overflow-x-auto border-t border-zinc-100 bg-white w-full hide-scrollbar">
        <router-link v-for="item in navItems" :key="item.path" :to="`/${route.params.slug}/admin/${item.path}`"
          ...
          :style="isActive(item.path) ? { color: tenant?.primary_color, borderBottomColor: tenant?.primary_color } : {}">
          <component :is="item.icon" class="w-5 h-5" />
          {{ item.name }}
        </router-link>

        <button @click="handleLogout"
          class="flex-none min-w-[80px] flex flex-col items-center justify-center gap-1.5 py-3 px-3 text-[10px] font-bold uppercase tracking-wider transition-colors border-b-2 border-transparent text-red-500 hover:text-red-600 hover:bg-red-50">
