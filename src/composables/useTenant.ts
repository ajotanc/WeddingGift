...
// Shared global state for the active tenant view
...
export function useTenant() {
	...
	const { sanitizeTenant } = authStore;
	...
	const fetchTenant = async (slug: string) => {
		...
		try {
			...
			if (data) {
				...
				music.musicUrl = tenant.value?.music_url ?? null;
				music.isPremium = tenant.value?.is_premium ?? false;
			} else {
